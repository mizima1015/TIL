...
# docstring을 잘 정리해야한다. price가 무엇인지 total_sales가 무엇인지
...
def main():
    ...
    price = st.slider("단가 :" , 1000, 10000, value= 50000)
    total_sales = st.slider("전체 판매 갯수", 1,1000, value=500)

    # 중간중간 값이 어떻게 바뀌는지 확인하고 싶을때
    ...
    col_name = st.selectbox('1개의 종을 전택하세요!',iris['species'].unique())
    st.write(col_name)
